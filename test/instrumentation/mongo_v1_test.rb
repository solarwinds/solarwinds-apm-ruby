...
if Gem.loaded_specs['mongo'].version.to_s < '2.0.0'
  describe "Mongo" do
    ...
    it 'Mongo should have appoptics methods defined' do
      AppOptics::Inst::Mongo::DB_OPS.each do |m|
        ::Mongo::DB.method_defined?("#{m}_with_appoptics").must_equal true
      end
      AppOptics::Inst::Mongo::CURSOR_OPS.each do |m|
        ::Mongo::Cursor.method_defined?("#{m}_with_appoptics").must_equal true
      end
      AppOptics::Inst::Mongo::COLL_WRITE_OPS.each do |m|
        ::Mongo::Collection.method_defined?("#{m}_with_appoptics").must_equal true
      end
      AppOptics::Inst::Mongo::COLL_QUERY_OPS.each do |m|
        ::Mongo::Collection.method_defined?("#{m}_with_appoptics").must_equal true
      end
      AppOptics::Inst::Mongo::COLL_INDEX_OPS.each do |m|
        ::Mongo::Collection.method_defined?("#{m}_with_appoptics").must_equal true
      end
    ...
    it "should trace update" do
      ...
      traces[3].has_key?('Backtrace').must_equal AppOptics::Config[:mongo][:collect_backtraces]
    ...
    it "should trace create, ensure and drop index" do
      ...
      traces[5].has_key?('Backtrace').must_equal AppOptics::Config[:mongo][:collect_backtraces]
