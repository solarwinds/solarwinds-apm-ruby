...
describe "Memcache" do
  before do
    ...
    @collect_backtraces = AppOptics::Config[:memcache][:collect_backtraces]
  end

  after do
    AppOptics::Config[:memcache][:collect_backtraces] = @collect_backtraces
  end
  ...
  it 'MemCache should have appoptics methods defined' do
  ...
  it "should trace set" do
    AppOptics::API.start_trace('memcache_test', '', {}) do
      @mc.set('msg', 'blah')
    end
    ...
    traces[1].has_key?('Backtrace').must_equal AppOptics::Config[:memcache][:collect_backtraces]

    validate_event_keys(traces[2], @info_kvs)
    traces[2]['KVKey'].must_equal "msg"
    traces[2].has_key?('Backtrace').must_equal AppOptics::Config[:memcache][:collect_backtraces]
  ...
  it "should trace get" do
    ...
    traces[3].has_key?('Backtrace').must_equal AppOptics::Config[:memcache][:collect_backtraces]
  ...
  it "should obey :collect_backtraces setting when true" do
    AppOptics::Config[:memcache][:collect_backtraces] = true
