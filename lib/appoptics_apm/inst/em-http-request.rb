        ...
        def setup_request_with_appoptics(*args, &block)
          ...
          if SolarWindsAPM.tracing?
            ...
            begin
              ...
              SolarWindsAPM.logger.debug "[appoptics_apm/debug] em-http-request KV error: #{e.inspect}"
            end
        ...
        def parse_response_header_with_appoptics(*args, &block)
          ...
          if SolarWindsAPM::TraceString.valid?(tracestring) && SolarWindsAPM.tracing?
            ...
            if trace_id == SolarWindsAPM::TraceString.trace_id(tracestring)
              ...
              SolarWindsAPM.logger.debug "[appoptics_apm/em-http] Mismatched returned X-Trace ID : #{tracestring}"
            end
          end
        ensure
...
if defined?(EventMachine::HttpConnection) && defined?(EventMachine::HttpClient) && SolarWindsAPM::Config[:em_http_request][:enabled]
  SolarWindsAPM.logger.info '[appoptics_apm/loading] Instrumenting em-http-request' if SolarWindsAPM::Config[:verbose]
  ...
  class EventMachine::HttpConnection
    ...
    if method_defined?(:setup_request)
      ...
      SolarWindsAPM.logger.warn '[appoptics_apm/loading] Couldn\'t properly instrument em-http-request (:setup_request).  Partial traces may occur.'
    end
  end
  ...
  class EventMachine::HttpClient
    ...
    if method_defined?(:parse_response_header)
      ...
      SolarWindsAPM.logger.warn '[appoptics_apm/loading] Couldn\'t properly instrument em-http-request (:parse_response_header).  Partial traces may occur.'
    end
  end
end
