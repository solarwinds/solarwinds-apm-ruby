...
if defined?(Cassandra) && Cassandra.is_a?(Class) && SolarWindsAPM::Config[:cassandra][:enabled]
  SolarWindsAPM.logger.info '[appoptics_apm/loading] Instrumenting cassandra' if SolarWindsAPM::Config[:verbose]
  ...
  class Cassandra
    ...
    [:insert, :remove, :count_columns, :get_columns, :multi_get_columns, :get,
     ...
     :add_keyspace, :drop_keyspace].each do |m|
      if method_defined?(m)
        ...
        SolarWindsAPM.logger.warn "[appoptics_apm/loading] Couldn't properly instrument Cassandra (#{m}).  Partial traces may occur."
      end
    end
    ...
    if method_defined?(:exists?)
      ...
      SolarWindsAPM.logger.warn '[appoptics_apm/loading] Couldn\'t properly instrument Cassandra (exists?).  Partial traces may occur.'
    end
  end # class Cassandra
