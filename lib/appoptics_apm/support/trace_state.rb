  ...
  module TraceState
    class << self
      ...
      def add_sw_member(tracestate, value)
        ...
        result = "#{SW_AMP_TRACESTATE_ID}=#{value}#{remove_sw(tracestate)}"

        if result.bytesize > SW_AMP_MAX_TRACESTATE_BYTES
      ...
      def sw_member_value(tracestate)
        regex = /^.*(#{SW_AMP_TRACESTATE_ID}=(?<sw_member_value>[a-f0-9]{16}-[a-f0-9]{2})).*$/.freeze
      ...
      def remove_sw(tracestate)
        ...
        tracestate.gsub!(/,{0,1}\s*#{SW_AMP_TRACESTATE_ID}=[^,]*/, '')
      ...
      def reduce_size(tracestate)
        ...
        large_members = members.select { |m| m.bytesize > SW_AMP_MAX_TRACESTATE_MEMBER_BYTES }
        while large_members[0] && size > SW_AMP_MAX_TRACESTATE_BYTES
          size -= large_members[0].bytesize + 1 # add 1 for comma
          members.delete(large_members.shift)
        end

        tracestate = members.reverse.join(',')
        until tracestate.bytesize <= SW_AMP_MAX_TRACESTATE_BYTES do
          tracestate.gsub!(/,[^,]*$/, '')
        end
        tracestate
      end
