...
##
# This module is the base module for the various implementations of AppOptics reporting.
# Current variations as of 2014-09-10 are a c-extension, JRuby (using AppOptics Java
...
module AppOpticsBase
  ...
  # between the Ruby and JAppOptics instrumentation under JRuby.
  #
  ...
  # JAppOptics has already initiated tracing.  In this case, we shouldn't
  ...
  # of JRuby, a trace already started by JAppOptics.
  ...
  def self.extended(cls)
    ...
    AppOptics.methods.select { |m| m =~ /^is_|^has_/ }.each do |c|
      unless c =~ /\?$|=$/
        # AppOptics.logger.debug "aliasing #{c}? to #{c}"
        alias_method "#{c}?", c
      end
    end
  end
  ...
  def pickup_context?(xtrace)
    return false unless AppOptics::XTrace.valid?(xtrace)

    if defined?(JRUBY_VERSION) && AppOptics.tracing?
      return false
    else
      return true
    end
  ...
  def tracing_layer_op?(operation)
    if operation.is_a?(Array)
      return operation.include?(AppOptics.layer_op)
    else
      return AppOptics.layer_op == operation.to_sym
    end
  end
  ...
  #
  def always?
    AppOptics::Config[:tracing_mode] &&
      AppOptics::Config[:tracing_mode].to_sym == :always
  end
  ...
  def never?
    AppOptics::Config[:tracing_mode] &&
      AppOptics::Config[:tracing_mode].to_sym == :never
  end
