...
module AppOptics
  module API
    ...
    module Profiling
      ...
      #
      #   def computation(n)
      #     AppOptics::API.profile('fib', { :n => n }) do
      #       fib(n)
      #     end
      #   end
      #
      ...
      #
      #   AppOptics::API.profile_method(Array, :sort, opts)
      #
      def profile_method(klass, method, opts = {}, extra_kvs = {})
        ...
        return true unless AppOptics.loaded

        if RUBY_VERSION < '1.9.3'
          AppOptics.logger.warn '[appoptics/error] profile_method: Use the legacy method profiling for Ruby versions before 1.9.3'
          return false

        elsif !klass.is_a?(Module)
          AppOptics.logger.warn "[appoptics/error] profile_method: Not sure what to do with #{klass}.  Send a class or module."
          return false
          ...
          if method.is_a?(String)
            ...
            AppOptics.logger.warn "[appoptics/error] profile_method: Not sure what to do with #{method}.  Send a string or symbol for method."
            return false
          end
        end
        ...
        if !instance_method && !class_method
          AppOptics.logger.warn "[appoptics/error] profile_method: Can't instrument #{klass}.#{method} as it doesn't seem to exist."
          AppOptics.logger.warn "[appoptics/error] #{__FILE__}:#{__LINE__}"
          return false
        end
        ...
        if klass.instance_methods.include?(with_appoptics.to_sym) ||
          ...
          AppOptics.logger.warn "[appoptics/error] profile_method: #{klass}::#{method} already profiled."
          AppOptics.logger.warn "[appoptics/error] profile_method: #{__FILE__}:#{__LINE__}"
          return false
        end

        source_location = []
        if instance_method
          ::AppOptics::Util.send_include(klass, ::AppOptics::MethodProfiling)
          source_location = klass.instance_method(method).source_location
        elsif class_method
          ::AppOptics::Util.send_extend(klass, ::AppOptics::MethodProfiling)
