...
module AppOptics
  module API
    ...
    module Util
      ...
      # keys are either keys that are handled by liboboe calls or the appoptics gem.
      #
      ...
      def blacklisted?(addr_port)
        return false unless AppOptics::Config.blacklist

        # Ensure that the blacklist is an array
        unless AppOptics::Config.blacklist.is_a?(Array)
          val = AppOptics::Config[:blacklist]
          AppOptics::Config[:blacklist] = [val.to_s]
        end

        AppOptics::Config.blacklist.each do |h|
