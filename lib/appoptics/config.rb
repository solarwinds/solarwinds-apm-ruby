...
module AppOptics
  ...
  # configure and/or modify the functionality of the appoptics gem.
  #
  # Use AppOptics::Config.show to view the entire nested hash.
  #
  module Config
    @@config = {}
    ...
    def self.initialize(_data = {})
      ...
      AppOptics::Config[:em_http_request][:enabled] = false

      # Set collect_backtraces defaults
      AppOptics::Config[:action_controller][:collect_backtraces] = false
      AppOptics::Config[:action_controller_api][:collect_backtraces] = false
      AppOptics::Config[:active_record][:collect_backtraces] = true
      AppOptics::Config[:bunnyclient][:collect_backtraces] = false
      AppOptics::Config[:bunnyconsumer][:collect_backtraces] = false
      ...
      AppOptics::Config[:excon][:collect_backtraces] = true
      AppOptics::Config[:faraday][:collect_backtraces] = false
      AppOptics::Config[:grape][:collect_backtraces] = true
      AppOptics::Config[:httpclient][:collect_backtraces] = true
      ...
      AppOptics::Config[:rack][:collect_backtraces] = false
      AppOptics::Config[:redis][:collect_backtraces] = false
      AppOptics::Config[:resqueclient][:collect_backtraces] = true
      AppOptics::Config[:resqueworker][:collect_backtraces] = false
      ...
      # Setup an empty host blacklist (see: AppOptics::API::Util.blacklisted?)
      @@config[:blacklist] = []
      ...
      @@config[:verbose] = ENV.key?('APPOPTICS_GEM_VERBOSE') ? true : false
    end
    ...
    def self.[](key)
      if key == :resque
        AppOptics.logger.warn '[appoptics/warn] :resque config is deprecated.  It is now split into :resqueclient and :resqueworker.'
        AppOptics.logger.warn "[appoptics/warn] Called from #{Kernel.caller[0]}"
      end
    ...
    def self.[]=(key, value)
      ...
      if key == :sampling_rate
        AppOptics.logger.warn 'sampling_rate is not a supported setting for AppOptics::Config.  ' \
                         'Please use :sample_rate.'

      elsif key == :sample_rate
        unless value.is_a?(Integer) || value.is_a?(Float)
          fail 'appoptics :sample_rate must be a number between 0 and 1000000 (1m)'
        end

        # Validate :sample_rate value
        unless value.between?(0, 1e6)
          fail 'appoptics :sample_rate must be between 0 and 1000000 (1m)'
        end

        # Assure value is an integer
        @@config[key.to_sym] = value.to_i
        AppOptics.set_sample_rate(value) if AppOptics.loaded

      elsif key == :action_blacklist
        AppOptics.logger.warn "[appoptics/unsupported] :action_blacklist has been deprecated and no longer functions."
      ...
      if key == :tracing_mode
        AppOptics.set_tracing_mode(value.to_sym) if AppOptics.loaded
