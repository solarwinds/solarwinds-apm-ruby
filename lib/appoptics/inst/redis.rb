    ...
    module Redis
      module Client
        ...
        def self.included(klass)
          ...
          # operations
          ::AppOptics::Util.method_alias(klass, :call, ::Redis::Client)
          ::AppOptics::Util.method_alias(klass, :call_pipeline, ::Redis::Client)
        end
        ...
        def extract_trace_details(command, r)
          ...
          if KV_COLLECT_MAP[op]
            ...
            when :script
              ...
              kvs[:Backtrace] = AppOptics::API.backtrace if AppOptics::Config[:redis][:collect_backtraces]
              if command[1] == 'load'
                if command[1].length > 1024
          ...
          AppOptics.logger.debug "Error collecting redis KVs: #{e.message}"
        ...
        def extract_pipeline_details(pipeline)
          ...
          AppOptics.logger.debug "[appoptics/debug] Error extracting pipelined commands: #{e.message}"
        ...
        def call_with_appoptics(command, &block)
          if AppOptics.tracing?
            ...
            begin
              ...
              ::AppOptics::API.log_exception(:redis, e)
              raise
            ensure
              ::AppOptics::API.log_exit(:redis, report_kvs)
            end
...
if AppOptics::Config[:redis][:enabled]
  if defined?(::Redis) && Gem::Version.new(::Redis::VERSION) >= Gem::Version.new('3.0.0')
    AppOptics.logger.info '[appoptics/loading] Instrumenting redis' if AppOptics::Config[:verbose]
    ::AppOptics::Util.send_include(::Redis::Client, ::AppOptics::Inst::Redis::Client)
  end
end
