...
if defined?(::Mongo) && (Gem.loaded_specs['mongo'].version.to_s < '2.0.0') && AppOptics::Config[:mongo][:enabled]
  AppOptics.logger.info '[appoptics/loading] Instrumenting mongo' if AppOptics::Config[:verbose]

  if defined?(::Mongo::DB)
    module ::Mongo
      class DB
        include AppOptics::Inst::Mongo

        # Instrument DB operations
        AppOptics::Inst::Mongo::DB_OPS.reject { |m| !method_defined?(m) }.each do |m|
          define_method("#{m}_with_appoptics") do |*args|
            report_kvs = {}

            begin
              report_kvs[:Flavor] = AppOptics::Inst::Mongo::FLAVOR
              ...
              report_kvs[:Backtrace] = AppOptics::API.backtrace if AppOptics::Config[:mongo][:collect_backtraces]
            rescue => e
  ...
  if defined?(::Mongo::Cursor)
    module ::Mongo
      class Cursor
        ...
        AppOptics::Inst::Mongo::CURSOR_OPS.reject { |m| !method_defined?(m) }.each do |m|
          define_method("#{m}_with_appoptics") do |*args|
            report_kvs = {}
    ...
    module ::Mongo
      class Collection
        ...
        def appoptics_collect(m, args)
          kvs = {}
          kvs[:Flavor] = AppOptics::Inst::Mongo::FLAVOR
          ...
          AppOptics.logger.debug "[appoptics/debug] Exception in appoptics_collect KV collection: #{e.inspect}"
        ensure
          return kvs
        end

        # Instrument Collection write operations
        AppOptics::Inst::Mongo::COLL_WRITE_OPS.reject { |m| !method_defined?(m) }.each do |m|
          define_method("#{m}_with_appoptics") do |*args|
        ...
        AppOptics::Inst::Mongo::COLL_QUERY_OPS.reject { |m| !method_defined?(m) }.each do |m|
        ...
        AppOptics::Inst::Mongo::COLL_INDEX_OPS.reject { |m| !method_defined?(m) }.each do |m|
          define_method("#{m}_with_appoptics") do |*args|
            report_kvs = appoptics_collect(m, args)
