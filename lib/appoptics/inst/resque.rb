  ...
  module Inst
    module ResqueClient
      def self.included(klass)
        ...
        ::AppOptics::Util.method_alias(klass, :enqueue, ::Resque)
        ::AppOptics::Util.method_alias(klass, :enqueue_to, ::Resque)
        ::AppOptics::Util.method_alias(klass, :dequeue, ::Resque)
      end
      ...
      def extract_trace_details(op, klass, args)
        ...
        begin
          ...
          if AppOptics::Config[:resqueclient][:log_args]
          ...
          report_kvs[:Backtrace] = AppOptics::API.backtrace if AppOptics::Config[:resqueclient][:collect_backtraces]
      ...
      def enqueue_with_appoptics(klass, *args)
        if AppOptics.tracing?
          ...
          AppOptics::API.trace(:'resque-client', report_kvs, :enqueue) do
      ...
      def enqueue_to_with_appoptics(queue, klass, *args)
        if AppOptics.tracing? && !AppOptics.tracing_layer_op?(:enqueue)
          report_kvs = extract_trace_details(:enqueue_to, klass, args)
          report_kvs[:Queue] = queue.to_s if queue

          AppOptics::API.trace(:'resque-client', report_kvs) do
    ...
    module ResqueWorker
      def self.included(klass)
        ::AppOptics::Util.method_alias(klass, :perform, ::Resque::Worker)
      end
      ...
      def perform_with_appoptics(job)
        ...
        begin
          ...
          if AppOptics::Config[:resqueworker][:log_args]
          ...
          report_kvs[:Backtrace] = AppOptics::API.backtrace if AppOptics::Config[:resqueworker][:collect_backtraces]
        rescue => e
          AppOptics.logger.debug "[appoptics/debug] #{__method__}:#{File.basename(__FILE__)}:#{__LINE__}: #{e.message}" if AppOptics::Config[:verbose]
        end

        AppOptics::API.start_trace(:'resque-worker', nil, report_kvs) do
    ...
    module ResqueJob
      def self.included(klass)
        ::AppOptics::Util.method_alias(klass, :fail, ::Resque::Job)
      end

      def fail_with_appoptics(exception)
        if AppOptics.tracing?
          AppOptics::API.log_exception(:resque, exception)
        end
...
if defined?(::Resque) && RUBY_VERSION >= '1.9.3'
  AppOptics.logger.info '[appoptics/loading] Instrumenting resque' if AppOptics::Config[:verbose]

  ::AppOptics::Util.send_include(::Resque,         ::AppOptics::Inst::ResqueClient)
  ::AppOptics::Util.send_include(::Resque::Worker, ::AppOptics::Inst::ResqueWorker)
  ::AppOptics::Util.send_include(::Resque::Job,    ::AppOptics::Inst::ResqueJob)
end
