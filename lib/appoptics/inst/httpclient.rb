  ...
  module Inst
    module HTTPClient
      def self.included(klass)
        ::AppOptics::Util.method_alias(klass, :do_request, ::HTTPClient)
        ::AppOptics::Util.method_alias(klass, :do_request_async, ::HTTPClient)
        ::AppOptics::Util.method_alias(klass, :do_get_stream, ::HTTPClient)
      end
      ...
      def appoptics_collect(method, uri, query = nil)
        ...
        if AppOptics::Config[:httpclient][:log_args]
          if query
            kvs[:RemoteURL] = uri.to_s + '?' + AppOptics::Util.to_query(query)
          else
        ...
        kvs[:HTTPMethod] = ::AppOptics::Util.upcase(method)
        kvs[:Backtrace] = AppOptics::API.backtrace if AppOptics::Config[:httpclient][:collect_backtraces]
        kvs
      rescue => e
        AppOptics.logger.debug "[appoptics/debug] Error capturing httpclient KVs: #{e.message}"
      ...
      def do_request_with_appoptics(method, uri, query, body, header, &block)
        ...
        blacklisted = AppOptics::API.blacklisted?(uri.hostname)
        ...
        begin
          ...
          AppOptics::API.log_entry(:httpclient, kvs)
          kvs.clear
          ...
          AppOptics::API.log_exception(:httpclient, e)
          raise e
        ensure
          AppOptics::API.log_exit(:httpclient, kvs)
        end
      end
      ...
      def do_get_stream_with_appoptics(req, proxy, conn)
        AppOptics::Context.fromString(req.header['X-Trace'].first) unless req.header['X-Trace'].empty?
        # Avoid cross host tracing for blacklisted domains
        ...
        begin
          ...
          # AppOptics::API.log_exit('httpclient', kvs.merge('Async' => 1))
          AppOptics::API.log_exit(:httpclient, kvs)
        end
      end
      ...
      def add_xtrace_header(headers)
        ...
        return nil unless AppOptics::XTrace.valid?(req_context)
...
if AppOptics::Config[:httpclient][:enabled] && defined?(::HTTPClient)
  ::AppOptics.logger.info '[appoptics/loading] Instrumenting httpclient' if AppOptics::Config[:verbose]
  ::AppOptics::Util.send_include(::HTTPClient, ::AppOptics::Inst::HTTPClient)
end
