    ...
    module Cassandra
      def extract_trace_details(op, column_family, keys, args, options = {})
        ...
        begin
          ...
          report_kvs[:Backtrace] = AppOptics::API.backtrace if AppOptics::Config[:cassandra][:collect_backtraces]
      ...
      def insert_with_appoptics(column_family, key, hash, options = {})
        return insert_without_appoptics(column_family, key, hash, options = {}) unless AppOptics.tracing?

        report_kvs = extract_trace_details(:insert, column_family, key, hash, options)

        AppOptics::API.trace(:cassandra, report_kvs) do
      ...
      def remove_with_appoptics(column_family, key, *columns_and_options)
        ...
        return send :remove_without_appoptics, *args unless AppOptics.tracing?
      ...
      def count_columns_with_appoptics(column_family, key, *columns_and_options)
        ...
        return send :count_columns_without_appoptics, *args unless AppOptics.tracing?
      ...
      def get_columns_with_appoptics(column_family, key, *columns_and_options)
        ...
        if AppOptics.tracing? && !AppOptics.tracing_layer_op?(:multi_get_columns)
      ...
      def multi_get_columns_with_appoptics(column_family, key, *columns_and_options)
        ...
        return send :multi_get_columns_without_appoptics, *args unless AppOptics.tracing?

        report_kvs = extract_trace_details(:multi_get_columns, column_family, key, columns_and_options)

        AppOptics::API.trace(:cassandra, report_kvs, :multi_get_columns) do
      ...
      def get_with_appoptics(column_family, key, *columns_and_options)
        ...
        return send :get_without_appoptics, *args unless AppOptics.tracing?

        report_kvs = extract_trace_details(:get, column_family, key, columns_and_options)

        AppOptics::API.trace(:cassandra, report_kvs, :get) do
      ...
      def multi_get_with_appoptics(column_family, key, *columns_and_options)
        ...
        if AppOptics.tracing? && !AppOptics.tracing_layer_op?(:get)
      ...
      def exists_with_appoptics?(column_family, key, *columns_and_options)
        ...
        return send :exists_without_appoptics?, *args unless AppOptics.tracing?
      ...
      def get_range_single_with_appoptics(column_family, options = {})
        if AppOptics.tracing? && !AppOptics.tracing_layer_op?(:get_range_batch)
      ...
      def get_range_batch_with_appoptics(column_family, options = {})
        return get_range_batch_without_appoptics(column_family, options) unless AppOptics.tracing?

        report_kvs = extract_trace_details(:get_range_batch, column_family, nil, nil)

        AppOptics::API.trace(:cassandra, report_kvs, :get_range_batch) do
      ...
      def get_indexed_slices_with_appoptics(column_family, index_clause, *columns_and_options)
        ...
        return send :get_indexed_slices_without_appoptics, *args unless AppOptics.tracing?
      ...
      def drop_index_with_appoptics(keyspace, column_family, column_name)
        return drop_index_without_appoptics(keyspace, column_family, column_name) unless AppOptics.tracing?
      ...
      def add_column_family_with_appoptics(cf_def)
        return add_column_family_without_appoptics(cf_def) unless AppOptics.tracing?
      ...
      def drop_column_family_with_appoptics(column_family)
        return drop_column_family_without_appoptics(column_family) unless AppOptics.tracing?
      ...
      def add_keyspace_with_appoptics(ks_def)
        return add_keyspace_without_appoptics(ks_def) unless AppOptics.tracing?
      ...
      def drop_keyspace_with_appoptics(keyspace)
        return drop_keyspace_without_appoptics(keyspace) unless AppOptics.tracing?
...
if defined?(::Cassandra) && ::Cassandra.is_a?(Class) && AppOptics::Config[:cassandra][:enabled]
  AppOptics.logger.info '[appoptics/loading] Instrumenting cassandra' if AppOptics::Config[:verbose]

  class ::Cassandra
    include AppOptics::Inst::Cassandra
    ...
    [:insert, :remove, :count_columns, :get_columns, :multi_get_columns, :get,
     ...
     :add_keyspace, :drop_keyspace].each do |m|
      ...
      else AppOptics.logger.warn "[appoptics/loading] Couldn't properly instrument Cassandra (#{m}).  Partial traces may occur."
      end
    end
    ...
    else AppOptics.logger.warn '[appoptics/loading] Couldn\'t properly instrument Cassandra (exists?).  Partial traces may occur.'
    end
  end # class Cassandra
end
