...
module AppOptics
  module Inst
    ...
    module MopedDatabase
      include AppOptics::Inst::Moped

      def self.included(klass)
        AppOptics::Inst::Moped::DB_OPS.each do |m|
          ::AppOptics::Util.method_alias(klass, m)
        end
      end

      def extract_trace_details(op)
        report_kvs = {}
        report_kvs[:Flavor] = AppOptics::Inst::Moped::FLAVOR
        ...
        report_kvs[:Backtrace] = AppOptics::API.backtrace if AppOptics::Config[:moped][:collect_backtraces]
      ...
      def command_with_appoptics(command)
        if AppOptics.tracing? && !AppOptics.layer_op && command.key?(:mapreduce)
          begin
          ...
          AppOptics::API.trace(:mongo, report_kvs) do
      ...
      def drop_with_appoptics
        return drop_without_appoptics unless AppOptics.tracing?
    ...
    module MopedIndexes
      ...
      def create_with_appoptics(key, options = {})
        return create_without_appoptics(key, options) unless AppOptics.tracing?
        ...
        AppOptics::API.trace(:mongo, report_kvs, :create_index) do
          create_without_appoptics(key, options = {})
        end
      end

      def drop_with_appoptics(key = nil)
        return drop_without_appoptics(key) unless AppOptics.tracing?
    ...
    module MopedQuery
      ...
      def count_with_appoptics
        return count_without_appoptics unless AppOptics.tracing?
      ...
      def sort_with_appoptics(sort)
        return sort_without_appoptics(sort) unless AppOptics.tracing?
      ...
      def limit_with_appoptics(limit)
        if AppOptics.tracing? && !AppOptics.tracing_layer_op?(:explain)
          begin
      ...
      def distinct_with_appoptics(key)
        return distinct_without_appoptics(key) unless AppOptics.tracing?
      ...
      def update_with_appoptics(change, flags = nil)
        if AppOptics.tracing? && !AppOptics.tracing_layer_op?([:update_all, :upsert])
          begin
      ...
      def update_all_with_appoptics(change)
        return update_all_without_appoptics(change) unless AppOptics.tracing?
        ...
        AppOptics::API.trace(:mongo, report_kvs, :update_all) do
          update_all_without_appoptics(change)
        end
      end

      def upsert_with_appoptics(change)
        return upsert_without_appoptics(change) unless AppOptics.tracing?
        ...
        AppOptics::API.trace(:mongo, report_kvs, :upsert) do
          upsert_without_appoptics(change)
        end
      end

      def explain_with_appoptics
        return explain_without_appoptics unless AppOptics.tracing?
        ...
        AppOptics::API.trace(:mongo, report_kvs, :explain) do
          explain_without_appoptics
        end
      end

      def modify_with_appoptics(change, options = {})
        return modify_without_appoptics(change, options) unless AppOptics.tracing?
      ...
      def remove_with_appoptics
        return remove_without_appoptics unless AppOptics.tracing?
      ...
      def remove_all_with_appoptics
        return remove_all_without_appoptics unless AppOptics.tracing?
    ...
    module MopedCollection
      ...
      def find_with_appoptics(selector = {})
        return find_without_appoptics(selector) unless AppOptics.tracing?
      ...
      def indexes_with_appoptics
        return indexes_without_appoptics unless AppOptics.tracing?
      ...
      def insert_with_appoptics(documents, flags = nil)
        if AppOptics.tracing? && !AppOptics.tracing_layer_op?(:create_index)
      ...
      def aggregate_with_appoptics(*pipeline)
        return aggregate_without_appoptics(*pipeline) unless AppOptics.tracing?
...
if defined?(::Moped) && AppOptics::Config[:moped][:enabled]
  ::AppOptics.logger.info '[appoptics/loading] Instrumenting moped' if AppOptics::Config[:verbose]
  ::AppOptics::Util.send_include(::Moped::Database,   ::AppOptics::Inst::MopedDatabase)
  ::AppOptics::Util.send_include(::Moped::Collection, ::AppOptics::Inst::MopedCollection)
  ::AppOptics::Util.send_include(::Moped::Query,      ::AppOptics::Inst::MopedQuery)
  ::AppOptics::Util.send_include(::Moped::Indexes,    ::AppOptics::Inst::MopedIndexes)
end
