    ...
    class << self
      ...
      def method_alias(cls, method, name = nil)
        ...
        if cls.method_defined?(method.to_sym) || cls.private_method_defined?(method.to_sym)
          ...
          AppOptics.logger.warn "[appoptics/loading] Couldn't properly instrument #{name}.#{method}.  Partial traces may occur."
        end
      end
      ...
      def class_method_alias(cls, method, name = nil)
        ...
        if cls.singleton_methods.include? method.to_sym
          ...
          AppOptics.logger.warn "[appoptics/loading] Couldn't properly instrument #{name}.  Partial traces may occur."
        end
      end
      ...
      def static_asset?(path)
        (path =~ Regexp.new(AppOptics::Config[:dnt_regexp], AppOptics::Config[:dnt_opts]))
      end
      ...
      def upcase(o)
        if o.is_a?(String) || o.respond_to?(:to_s)
          ...
          AppOptics.logger.debug "[appoptics/debug] AppOptics::Util.upcase: could not convert #{o.class}"
          'UNKNOWN'
        end
      end
      ...
      # via AppOptics::Config[:sanitize_sql_regexp] and
      # AppOptics::Config[:sanitize_sql_opts].
      #
      def sanitize_sql(sql)
        return sql unless AppOptics::Config[:sanitize_sql]

        regexp = Regexp.new(AppOptics::Config[:sanitize_sql_regexp], AppOptics::Config[:sanitize_sql_opts])
        sql.gsub(regexp, '?')
      end
      ...
      # and for AppOptics.support_report.
      #
      ...
      def legacy_build_init_report
        ...
        begin
          ...
          AppOptics.logger.warn "[appoptics/warn] Error in legacy_build_init_report: #{e.message}"
      ...
      def build_init_report
        ...
        begin
          ...
          platform_info['Ruby.AppOptics.Version']       = ::AppOptics::Version::STRING
          # Should this be the oboe version, separate from the Ruby library's version?
          platform_info['Ruby.Oboe.Version']            = ::AppOptics::Version::STRING
          platform_info['RubyHeroku.AppOptics.Version'] = ::AppOpticsHeroku::Version::STRING if defined?(::AppOpticsHeroku)
          platform_info['Ruby.TraceMode.Version']       = ::AppOptics::Config[:tracing_mode]
          ...
          AppOptics.logger.warn "[appoptics/warn] Error in build_init_report: #{e.message}"
          AppOptics.logger.debug e.backtrace
        end
        platform_info
      end
