...
module AppOptics
  ...
  ##
  # This module houses all of the loading functionality for the appoptics gem.
  #
  ...
  module Loading
    ##
    # Load the appoptics tracing API
    #
    def self.require_api
      ...
      require 'appoptics/api'

      begin
        AppOptics::API.extend_with_tracing
      rescue LoadError => e
        AppOptics.logger.fatal "[appoptics/error] Couldn't load api: #{e.message}"
      end
    end
  end
end

AppOptics::Loading.require_api

# Auto-start the Reporter unless we running Unicorn on Heroku
# In that case, we start the reporters after fork
unless AppOptics.heroku? && AppOptics.forking_webserver?
  AppOptics::Reporter.start if AppOptics.loaded
end
