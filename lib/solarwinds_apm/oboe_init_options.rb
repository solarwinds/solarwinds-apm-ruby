...
module SolarWindsAPM
  ...
  class OboeInitOptions
    ...
    def read_and_validate_service_key
      ...
      unless service_key
        SolarWindsAPM.logger.error "[solarwinds_apm/oboe_options] SW_AMP_SERVICE_KEY not configured."
        return ''
      end
    ...
    def validate_token(token)
      if (token !~ /^[0-9a-zA-Z_-]{71}$/) && ENV['SW_AMP_COLLECTOR'] !~ /java-collector:1222/
        ...
        SolarWindsAPM.logger.error "[solarwinds_apm/oboe_options] SW_AMP_SERVICE_KEY problem. API Token in wrong format. Masked token: #{masked}"
        return false
      end
    ...
    def validate_transform_service_name(service_name)
      ...
      if service_name.empty?
        SolarWindsAPM.logger.error "[solarwinds_apm/oboe_options] SW_AMP_SERVICE_KEY problem. Service Name is missing"
        return false
      end
      ...
      if name != service_name
        SolarWindsAPM.logger.warn "[solarwinds_apm/oboe_options] SW_AMP_SERVICE_KEY problem. Service Name transformed from #{service_name} to #{name}"
        service_name = name
      end
    ...
    def read_and_validate_proxy
      ...
      unless proxy =~ /http:\/\/.*:\d+$/
        SolarWindsAPM.logger.error "[solarwinds_apm/oboe_options] SW_AMP_PROXY/http_proxy doesn't start with 'http://', #{proxy}"
