...
module SolarWindsAPM
  module Inst
    module Cassandra
      ...
      def insert_with_sw_apm(column_family, key, hash, options = {})
        return insert_without_sw_apm(column_family, key, hash, options = {}) unless SolarWindsAPM.tracing?
        ...
        SolarWindsAPM::SDK.trace(:cassandra, kvs: report_kvs) do
          insert_without_sw_apm(column_family, key, hash, options = {})
        end
      end

      def remove_with_sw_apm(column_family, key, *columns_and_options)
        args = [column_family, key] + columns_and_options

        return send :remove_without_sw_apm, *args unless SolarWindsAPM.tracing?

        report_kvs = extract_trace_details(:remove, column_family, key, columns_and_options)

        SolarWindsAPM::SDK.trace(:cassandra, kvs: report_kvs) do
          send :remove_without_sw_apm, *args
        end
      end

      def count_columns_with_sw_apm(column_family, key, *columns_and_options)
        args = [column_family, key] + columns_and_options

        return send :count_columns_without_sw_apm, *args unless SolarWindsAPM.tracing?

        report_kvs = extract_trace_details(:count_columns, column_family, key, columns_and_options)

        SolarWindsAPM::SDK.trace(:cassandra, kvs: report_kvs) do
          send :count_columns_without_sw_apm, *args
        end
      end

      def get_columns_with_sw_apm(column_family, key, *columns_and_options)
        args = [column_family, key] + columns_and_options
        ...
        if SolarWindsAPM.tracing? && !SolarWindsAPM.tracing_layer_op?(:multi_get_columns)
          ...
          SolarWindsAPM::SDK.trace(:cassandra, kvs: report_kvs) do
            send :get_columns_without_sw_apm, *args
          end
        else
          send :get_columns_without_sw_apm, *args
        end
      ...
      def multi_get_columns_with_sw_apm(column_family, key, *columns_and_options)
        args = [column_family, key] + columns_and_options

        return send :multi_get_columns_without_sw_apm, *args unless SolarWindsAPM.tracing?
        ...
        SolarWindsAPM::SDK.trace(:cassandra, kvs: report_kvs, protect_op: :multi_get_columns) do
          send :multi_get_columns_without_sw_apm, *args
        end
      end

      def get_with_sw_apm(column_family, key, *columns_and_options)
        args = [column_family, key] + columns_and_options

        return send :get_without_sw_apm, *args unless SolarWindsAPM.tracing?
        ...
        SolarWindsAPM::SDK.trace(:cassandra, kvs: report_kvs, protect_op: :get) do
          send :get_without_sw_apm, *args
        end
      end

      def multi_get_with_sw_apm(column_family, key, *columns_and_options)
        args = [column_family, key] + columns_and_options
        ...
        if SolarWindsAPM.tracing? && !SolarWindsAPM.tracing_layer_op?(:get)
          ...
          SolarWindsAPM::SDK.trace(:cassandra, kvs: report_kvs) do
            send :multi_get_without_sw_apm, *args
          end
        else
          send :multi_get_without_sw_apm, *args
        end
      ...
      def exists_with_sw_apm?(column_family, key, *columns_and_options)
        args = [column_family, key] + columns_and_options

        return send :exists_without_sw_apm?, *args unless SolarWindsAPM.tracing?

        report_kvs = extract_trace_details(:exists?, column_family, key, columns_and_options)

        SolarWindsAPM::SDK.trace(:cassandra, kvs: report_kvs) do
          send :exists_without_sw_apm?, *args
        end
      end

      def get_range_single_with_sw_apm(column_family, options = {})
        if SolarWindsAPM.tracing? && !SolarWindsAPM.tracing_layer_op?(:get_range_batch)
          ...
          SolarWindsAPM::SDK.trace(:cassandra, kvs: report_kvs) do
            get_range_single_without_sw_apm(column_family, options)
          end
        else
          get_range_single_without_sw_apm(column_family, options)
        end
      ...
      def get_range_batch_with_sw_apm(column_family, options = {})
        return get_range_batch_without_sw_apm(column_family, options) unless SolarWindsAPM.tracing?
        ...
        SolarWindsAPM::SDK.trace(:cassandra, kvs: report_kvs, protect_op: :get_range_batch) do
          get_range_batch_without_sw_apm(column_family, options)
        end
      end

      def get_indexed_slices_with_sw_apm(column_family, index_clause, *columns_and_options)
        args = [column_family, index_clause] + columns_and_options

        return send :get_indexed_slices_without_sw_apm, *args unless SolarWindsAPM.tracing?

        report_kvs = extract_trace_details(:get_indexed_slices, column_family, nil, columns_and_options)

        SolarWindsAPM::SDK.trace(:cassandra, kvs: report_kvs) do
          send :get_indexed_slices_without_sw_apm, *args
        end
      end

      def create_index_with_sw_apm(keyspace, column_family, column_name, validation_class)
        unless SolarWindsAPM.tracing?
          return create_index_without_sw_apm(keyspace, column_family, column_name, validation_class)
        end
        ...
        SolarWindsAPM::SDK.trace(:cassandra, kvs: report_kvs) do
          create_index_without_sw_apm(keyspace, column_family, column_name, validation_class)
        end
      end

      def drop_index_with_sw_apm(keyspace, column_family, column_name)
        return drop_index_without_sw_apm(keyspace, column_family, column_name) unless SolarWindsAPM.tracing?
        ...
        SolarWindsAPM::SDK.trace(:cassandra, kvs: report_kvs) do
          drop_index_without_sw_apm(keyspace, column_family, column_name)
        end
      end

      def add_column_family_with_sw_apm(cf_def)
        return add_column_family_without_sw_apm(cf_def) unless SolarWindsAPM.tracing?
        ...
        SolarWindsAPM::SDK.trace(:cassandra, kvs: report_kvs) do
          add_column_family_without_sw_apm(cf_def)
        end
      end

      def drop_column_family_with_sw_apm(column_family)
        return drop_column_family_without_sw_apm(column_family) unless SolarWindsAPM.tracing?

        report_kvs = extract_trace_details(:drop_column_family, column_family, nil, nil)

        SolarWindsAPM::SDK.trace(:cassandra, kvs: report_kvs) do
          drop_column_family_without_sw_apm(column_family)
        end
      end

      def add_keyspace_with_sw_apm(ks_def)
        return add_keyspace_without_sw_apm(ks_def) unless SolarWindsAPM.tracing?

        report_kvs = extract_trace_details(:add_keyspace, nil, nil, nil)
        report_kvs[:Name] = ks_def.name rescue ''

        SolarWindsAPM::SDK.trace(:cassandra, kvs: report_kvs) do
          add_keyspace_without_sw_apm(ks_def)
        end
      end

      def drop_keyspace_with_sw_apm(keyspace)
        return drop_keyspace_without_sw_apm(keyspace) unless SolarWindsAPM.tracing?

        report_kvs = extract_trace_details(:drop_keyspace, nil, nil, nil)
        report_kvs[:Name] = keyspace.to_s rescue ''

        SolarWindsAPM::SDK.trace(:cassandra, kvs: report_kvs) do
          drop_keyspace_without_sw_apm(keyspace)
        end
      end
    end
  end
...
if defined?(Cassandra) && Cassandra.is_a?(Class) && SolarWindsAPM::Config[:cassandra][:enabled]
  ...
  class Cassandra
    ...
    if method_defined?(:exists?)
      alias exists_without_sw_apm? exists?
      alias exists? exists_with_sw_apm?
    else
