  ...
  module Config
    ...
    def self.load_config_file
      ...
      if ENV.key?('SW_AMP_APM_CONFIG_RUBY')
        ...
        elsif File.exist?(File.join(ENV['SW_AMP_APM_CONFIG_RUBY'], 'solarwinds_apm_config.rb'))
          config_files << File.join(ENV['SW_AMP_APM_CONFIG_RUBY'], 'solarwinds_apm_config.rb')
        else
          SolarWindsAPM.logger.warn "[solarwinds_apm/config] Could not find the configuration file set by the SW_AMP_APM_CONFIG_RUBY environment variable:  #{ENV['SW_AMP_APM_CONFIG_RUBY']}"
        end
      end
